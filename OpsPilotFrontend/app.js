...
class OpsPilotApp {
    ...
    // === SSE 流式对话 ===
    //
    // 【AI 概念】SSE（Server-Sent Events）前端消费
    //
    // 后端通过 SSE 协议实时推送 Agent 的响应。前端使用 Fetch API 的
    // ReadableStream（而非 EventSource API）来消费 SSE 流，这样可以
    // 更灵活地控制请求（如发送 POST 请求体）。
    //
    // SSE 事件格式（来自后端 sse.go）：
    //   id: 1700000000000000000    ← 事件 ID
    //   event: message             ← 事件类型
    //   data: 你好                  ← 事件数据
    //   （空行）                    ← 事件分隔符
    //
    // 事件类型约定：
    //   "connected" → 连接已建立
    //   "message"   → Agent 响应片段（逐 token 推送，形成"打字机效果"）
    //   "done"      → 响应完成
    //   "error"     → 发生错误
    //
    // 发送流式消息
    async sendStreamMessage(message) {
        try {
            // 向后端 /api/chat_stream 发送 POST 请求
            // 后端 Controller（chat_v1_chat_stream.go）会：
            //   1. 创建 SSE 连接
            //   2. 构建 RAG Chat Agent Pipeline
            //   3. 流式执行 Agent，每个 token 通过 SSE 推送回来
            const response = await fetch(`${this.apiBaseUrl}/chat_stream`, {
                ...
                body: JSON.stringify({
                    Id: this.sessionId,      // 会话 ID（用于关联对话记忆）
            ...
            // 【AI 概念】为什么用 ReadableStream 而不是 EventSource？
            // EventSource API 只支持 GET 请求，无法发送 POST body。
            // 使用 Fetch + ReadableStream 可以发送 POST 请求并流式读取响应。
            const reader = response.body.getReader();
            const decoder = new TextDecoder();
            let buffer = '';        // 未解析的数据缓冲区
            ...
            try {
                while (true) {
                    ...
                    // 按换行符分割，逐行解析 SSE 事件
                    // SSE 协议中，每个字段占一行，事件之间用空行分隔
                    const lines = buffer.split('\n');
                    buffer = lines.pop() || ''; // 保留最后一行（可能不完整）

                    for (const line of lines) {
                        if (line.trim() === '') continue; // 跳过空行（SSE 事件分隔符）

                        // 解析 SSE 格式：每行以 "字段名: 值" 格式
                        if (line.startsWith('id: ')) {
                            ...
                            // 只处理 "message" 事件的数据（Agent 的逐 token 响应）
                            if (currentEvent === 'message') {
                                ...
                                if (data === '') {
                                    ...
                                    // 【AI 概念】逐 token 拼接
                                    // 每个 "message" 事件携带一个 token（LLM 生成的一个片段）
                                    // 拼接所有 token 形成完整回答，实现"打字机效果"
                                    fullResponse += data;
                                }

                                if (assistantMessageElement) {
                                    const messageContent = assistantMessageElement.querySelector('.message-content');
                                    // 流式过程中使用纯文本显示（避免频繁 Markdown 渲染影响性能）
                                    // 流结束后（done 事件）会切换为 Markdown 渲染
